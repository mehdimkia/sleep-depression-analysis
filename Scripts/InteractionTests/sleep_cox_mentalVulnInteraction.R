# --------------------------------------------------------------------
# sleep_cox_mentalVulnInteraction.R
#   Fully adjusted Cox model + sleep_dur2 × mental_vuln (MINI or medication) interaction
# --------------------------------------------------------------------
...
## 3 · Factorise & define mental vulnerability ------------------------
dat <- dat %>%
  mutate(
...
## 7 · Cox model with sleep × mental_vuln interaction -----------------
fit_int <- cph(
  Surv(LD_PHQ9depr_Surv_time_standard_default, LD_PHQ9depr_event) ~
    sleep_dur2 * mental_vuln +
    frag_break_q4 +
...
cat("\n--- LR test: sleep_dur2 × mental_vuln ---\n")
print(lrtest(fit_base, fit_int))

## 9 · Interaction coefficients & cross‐product HRs --------------------
cat("\n--- Interaction model coefficients (cph) ---\n")
print(summary(fit_int), digits=3)

# Refit with survival::coxph to extract stratum‐specific HRs
fit_int_coxph <- coxph(
  Surv(LD_PHQ9depr_Surv_time_standard_default, LD_PHQ9depr_event) ~
    sleep_dur2 * mental_vuln +
    frag_break_q4 +
...
cp <- tidy(fit_int_coxph, exponentiate=TRUE, conf.int=TRUE) %>%
  filter(grepl("sleep_dur2.*:mental_vulnYes", term))

cat("\n--- Stratum‐specific HRs for sleep by mental_vuln ---\n")
print(cp, digits=3)

## 10 · Plot predicted HR curves --------------------------------------

plot(
  Predict(fit_int, sleep_dur2, mental_vuln, fun=exp),
  xlab="Sleep duration", ylab="Hazard ratio",
  main="Interaction: Sleep × Mental Vulnerability"
)
